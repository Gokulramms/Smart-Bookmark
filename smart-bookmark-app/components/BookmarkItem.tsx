...
export function BookmarkItem({ bookmark, onDelete, viewMode = 'grid' }: BookmarkItemProps) {
    ...
    const categoryColors: Record<string, { from: string; to: string; glow: string }> = {
        Work: { from: 'from-blue-500', to: 'to-blue-700', glow: 'shadow-blue-500/50' },
        Personal: { from: 'from-purple-500', to: 'to-purple-700', glow: 'shadow-purple-500/50' },
        Research: { from: 'from-green-500', to: 'to-green-700', glow: 'shadow-green-500/50' },
        Shopping: { from: 'from-cyan-500', to: 'to-cyan-700', glow: 'shadow-cyan-500/50' },
        Entertainment: { from: 'from-orange-500', to: 'to-orange-700', glow: 'shadow-orange-500/50' },
        News: { from: 'from-red-500', to: 'to-red-700', glow: 'shadow-red-500/50' },
        Education: { from: 'from-indigo-500', to: 'to-indigo-700', glow: 'shadow-indigo-500/50' },
        Tools: { from: 'from-gray-500', to: 'to-gray-700', glow: 'shadow-gray-500/50' },
        Social: { from: 'from-yellow-500', to: 'to-yellow-700', glow: 'shadow-yellow-500/50' },
        Other: { from: 'from-slate-500', to: 'to-slate-700', glow: 'shadow-slate-500/50' },
    };
        ...
        <div className="group relative bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-2xl rounded-3xl border border-white/10 overflow-hidden hover:border-white/20 transition-all duration-500 transform hover:-translate-y-2 hover:shadow-2xl hover:shadow-purple-500/20">
            ...
            {showDeleteConfirm && (
                <div className="absolute inset-0 bg-black/90 backdrop-blur-xl z-20 flex items-center justify-center p-6 rounded-3xl">
                    <div className="bg-gradient-to-br from-red-950/80 to-red-900/80 backdrop-blur-2xl rounded-2xl p-8 max-w-xs w-full shadow-2xl border border-red-500/30">
                        <h3 className="text-2xl font-black mb-3 text-white">Delete Forever?</h3>
            ...
            <div className="relative p-7">
                ...
                <div className="flex items-start justify-between gap-4 mb-5">
                    <div className="flex items-center gap-4 min-w-0 flex-1">
                        <div className="relative flex-shrink-0 group/icon">
                            <div className="absolute inset-0 bg-gradient-to-br from-purple-500/30 to-blue-500/30 rounded-2xl blur-lg group-hover/icon:blur-xl transition-all" />
                            <img
                                src={bookmark.favicon_url || getFaviconUrl(bookmark.url)}
                                alt=""
                                className="relative w-14 h-14 rounded-2xl bg-white/10 p-2 border border-white/10 shadow-xl transform group-hover/icon:scale-110 transition-transform"
                                onError={(e) => {
                                    (e.target as HTMLImageElement).src = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="white"><path d="M12 2L2 7v10c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V7l-10-5z"/></svg>';
                                }}
                            />
                        </div>
                        <div className="min-w-0 flex-1">
                            <h3 className="font-black text-white truncate text-xl mb-2 group-hover:text-purple-300 transition-colors">
                                {bookmark.title}
                            </h3>
                            <a
                                href={bookmark.url}
                ...
                <div className="flex flex-wrap items-center gap-2 mb-5">
                    {bookmark.category && (
                        <span className={`px-4 py-2 rounded-xl text-xs font-black bg-gradient-to-r ${colors.from} ${colors.to} text-white shadow-lg ${colors.glow} uppercase tracking-wide`}>
                            {bookmark.category}
                        </span>
                    )}
                    {bookmark.tags && bookmark.tags.slice(0, 3).map((tag, index) => (
                        <span
                            key={index}
                            className="px-4 py-2 bg-white/10 backdrop-blur-xl text-white/90 rounded-xl text-xs font-bold border border-white/20 hover:bg-white/20 transition-all"
                        >
                            #{tag}
                        </span>
                    ))}
